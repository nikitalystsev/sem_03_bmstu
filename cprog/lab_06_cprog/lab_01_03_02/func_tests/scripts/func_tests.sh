...
ls ../data/ > ./../../file.txt
pos_in=()
while IFS=" " read -r line; do pos_in+=("$line"); done < <(grep "pos_[0-9][1-9]_in.txt" ./../../file.txt)

pos_out=()
while IFS=" " read -r line; do pos_out+=("$line"); done < <(grep "pos_[0-9][1-9]_out.txt" ./../../file.txt)

neg_in=()
while IFS=" " read -r line; do neg_in+=("$line"); done < <(grep "neg_[0-9][1-9]_in.txt" ./../../file.txt)
pos_args=()
neg_args=()
...
if [ -n "$1" ] && [ "$1" = "--USE-VALGRIND" ]; then
    ...
    do
        ...
        ./pos_case.sh "$1" ../data/"${pos_in[i]}" ../data/"${pos_out[i]}" "$app_args"
        check=$?
    ...
    do
        ...
        ./neg_case.sh "$1" ../data/"${neg_in[i]}" "$app_args"
        check=$?
